...
public class MenuBean implements Serializable {
    ...
    public MenuModel generarMenus() {
      ...
      System.out.println("roles--->"+mas );
            List<RolMenu> lstRolesXMenus = rolMenuFacade.findByIdrolMenu( mas );
            System.out.println("roles--->"+lstRolesXMenus);
            for(RolMenu rm :lstRolesXMenus) {
                String existeMenu= "";
                if (rm.getMenuIdmenu().getCorrelativo() == 0) {
                    firstSubmenu = new DefaultSubMenu(rm.getMenuIdmenu().getMenu());
                    ...
                    item = new DefaultMenuItem(rm.getMenuIdmenu().getMenu());
                    item.setId(FacesContext.getCurrentInstance().getViewRoot().createUniqueId());
                    item.setOutcome(rm.getMenuIdmenu().getUrl());
